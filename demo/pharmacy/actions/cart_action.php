...
if ($product_id > 0) {
    switch ($action) {
        case 'add':
            ...
            $stmt = $pdo->prepare("SELECT id, is_autoship FROM pharmacy_medicines WHERE id = ? AND stock > 0");
        ...
        case 'toggle_type':
            ...
            if ($current_type === 'autoship') {
                ...
                $stmt = $pdo->prepare("SELECT is_autoship FROM pharmacy_medicines WHERE id = ?");
                $stmt->execute([$product_id]);
