...
// Fetch dynamic pharmacy statistics
// 1. Orders Statistics
$stmt = $pdo->query("SELECT COUNT(*) as total, SUM(total_amount) as total_sales, SUM(status='pending') as pending FROM orders");
$order_stats = $stmt->fetch(PDO::FETCH_ASSOC);

// 2. Active Medicines
$stmt = $pdo->query("SELECT COUNT(*) as total, SUM(stock > 0) as in_stock FROM pharmacy_medicines");
$med_stats = $stmt->fetch(PDO::FETCH_ASSOC);

// 3. Cold Chain Medicines
$stmt = $pdo->query("SELECT COUNT(*) as total FROM pharmacy_medicines WHERE requires_cold_chain = 1");
$cold_chain_stats = $stmt->fetch(PDO::FETCH_ASSOC);

// 4. Low stock alert
$stmt = $pdo->query("SELECT COUNT(*) as low_stock FROM pharmacy_medicines WHERE stock <= 5");
$inv_stats = $stmt->fetch(PDO::FETCH_ASSOC);

// 5. Recent Pharmacy Orders
$stmt = $pdo->query("
...
// 6. Item needing inventory attention
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white p-6 rounded-xl stat-card-shadow border border-outline-variant/30 group hover:border-primary-container transition-colors">
            <div class="flex justify-between items-start mb-4">
                ...
                <span class="text-status-active font-label-sm flex items-center gap-1">
                    <?= (int)$order_stats['pending'] ?> در انتظار
                </span>
            </div>
            <h3 class="font-label-lg text-label-lg text-on-surface-variant">کل سفارشات داروخانه</h3>
            <p class="font-display-lg text-display-lg text-primary mt-1"><?= (int)$order_stats['total'] ?></p>
            <p class="text-label-sm text-on-surface-variant mt-2">فروش کل: <?= number_format((int)($order_stats['total_sales'] ?? 0)) ?> تومان</p>
        </div>

        <!-- Card: Active Medicines -->
        <div class="bg-white p-6 rounded-xl stat-card-shadow border border-outline-variant/30 group hover:border-secondary-container transition-colors">
            <div class="flex justify-between items-start mb-4">
                ...
                <span class="text-status-active font-label-sm flex items-center gap-1">
                    <?= (int)$med_stats['in_stock'] ?> موجود
                </span>
            </div>
            <h3 class="font-label-lg text-label-lg text-on-surface-variant">تعداد اقلام دارویی</h3>
            <p class="font-display-lg text-display-lg text-primary mt-1"><?= (int)$med_stats['total'] ?></p>
            <p class="text-label-sm text-on-surface-variant mt-2">دارو، واکسن و سرم‌های تخصصی</p>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl stat-card-shadow border border-outline-variant/30 group hover:border-primary-container transition-colors">
            <div class="flex justify-between items-start mb-4">
                <div class="w-12 h-12 rounded-lg bg-teal-50 flex items-center justify-center text-teal-600">
                    <span class="material-symbols-outlined text-[32px]">ac_unit</span>
                </div>
                <span class="text-teal-600 font-label-sm font-bold flex items-center gap-1">
                    ۲ تا ۸ درجه
                </span>
            </div>
            <h3 class="font-label-lg text-label-lg text-on-surface-variant">داروهای زنجیره سرد</h3>
            <div class="flex items-baseline gap-2 mt-1">
                <p class="font-display-lg text-display-lg text-primary"><?= (int)$cold_chain_stats['total'] ?></p>
                <span class="text-label-lg text-on-surface-variant">قلم دارو</span>
            </div>
            <p class="text-label-sm text-on-surface-variant mt-2">نیازمند عایق و پکیج یخ خشک</p>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl stat-card-shadow border border-outline-variant/30 group hover:border-error transition-colors">
            <div class="flex justify-between items-start mb-4">
                ...
                <?php if($inv_stats['low_stock'] > 0): ?>
                    <span class="text-error font-bold font-label-sm">کسری موجودی</span>
                <?php else: ?>
                    <span class="text-status-active font-bold font-label-sm">ایمن</span>
                <?php endif; ?>
            </div>
            <h3 class="font-label-lg text-label-lg text-on-surface-variant">هشدار موجودی دارو</h3>
            <p class="font-display-lg text-display-lg text-primary mt-1"><?= (int)$inv_stats['low_stock'] ?></p>
            <p class="text-label-sm text-on-surface-variant mt-2">داروهای زیر ۵ عدد موجودی</p>
        </div>
    </div>

    <!-- Main Layout Grid: Recent Orders & Pharmacy Operations -->
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <!-- Recent Orders (2/3 Column) -->
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-xl stat-card-shadow border border-outline-variant/30 overflow-hidden">
                <div class="px-6 py-5 border-b border-outline-variant/20 flex justify-between items-center">
                    <h2 class="font-title-lg text-title-lg text-primary">آخرین سفارشات دارویی</h2>
                    <a href="orders.php" class="text-primary-container font-label-lg hover:underline">مشاهده همه سفارشات</a>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-right">
                        <thead class="bg-surface-container-low text-on-surface-variant font-label-sm">
                            <tr>
                                <th class="px-6 py-3">شماره سفارش</th>
                                <th class="px-6 py-3">نام خریدار</th>
                        ...
                        <tbody class="divide-y divide-outline-variant/10">
                            <?php if (empty($recent_orders)): ?>
                                <tr><td colspan="5" class="px-6 py-4 text-center text-on-surface-variant">هیچ سفارشی ثبت نشده است.</td></tr>
                            <?php else: ?>
                                <?php foreach ($recent_orders as $ord): ?>
                                    <tr class="hover:bg-surface-container-lowest transition-colors">
                                        <td class="px-6 py-4 font-bold text-primary">#<?= (int)$ord['id'] ?></td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p class="font-label-lg text-primary"><?= htmlspecialchars($ord['customer_name'] ?? 'مشتری مهمان') ?></p>
                                                    <p class="text-xs text-on-surface-variant"><?= htmlspecialchars($ord['customer_phone'] ?? '') ?></p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 font-body-md text-primary font-bold"><?= number_format((int)$ord['total_amount']) ?> تومان</td>
                                        <td class="px-6 py-4 font-body-md text-on-surface-variant"><?= htmlspecialchars($ord['created_at']) ?></td>
                                        <td class="px-6 py-4">
                                            <?php 
                                            ...
                                            } elseif ($ord['status'] == 'completed' || $ord['status'] == 'paid') {
                                                $bg = 'bg-status-active/20 text-status-active';
                                                $label = 'پرداخت شده';
                                            } elseif ($ord['status'] == 'cancelled') {
                                                $bg = 'bg-error/20 text-error';
                                                $label = 'لغو شده';
                                            }
                                            ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="bg-white p-6 rounded-xl stat-card-shadow border border-outline-variant/30">
                    <h2 class="font-title-lg text-title-lg text-primary mb-6">آیتم نیازمند توجه در انبار دارویی</h2>
                    <div class="space-y-4">
                        <?php if($top_product): ?>
                            <?php 
